(function(){
  ...
  const mapProduct=r=>({id:r.id,nameAr:r.name_ar||"",nameEn:r.name_en||"",descAr:r.description_ar||"",descEn:r.description_en||"",category:r.category||"all",price:Number(r.price)||0,stock:Number(r.stock)||0,badgeAr:r.badge_ar||"",badgeEn:r.badge_en||"",visible:r.visible!==false,featured:!!r.featured,isPackage:!!r.is_package,showPrice:r.show_price!==false,showAddToCart:r.show_add_to_cart!==false,sort:Number(r.sort_order)||0,images:Array.isArray(r.images)?r.images:[]});
  const unmapProduct=p=>({name_ar:String(p.nameAr||"").trim(),name_en:String(p.nameEn||"").trim(),description_ar:String(p.descAr||"").trim(),description_en:String(p.descEn||"").trim(),category:p.category||"all",price:Math.max(0,Number(p.price)||0),stock:Math.max(0,Math.floor(Number(p.stock)||0)),badge_ar:String(p.badgeAr||"").trim(),badge_en:String(p.badgeEn||"").trim(),visible:p.visible!==false,featured:!!p.featured,is_package:!!p.isPackage,show_price:p.showPrice!==false,show_add_to_cart:p.showAddToCart!==false,sort_order:Math.floor(Number(p.sort)||0),images:Array.isArray(p.images)?p.images.filter(Boolean).slice(0,10):[],updated_at:new Date().toISOString()});
