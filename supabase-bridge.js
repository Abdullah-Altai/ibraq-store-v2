(function(){
  ...
  window.ibraqCloud={
    ...
    async uploadMedia(file){if(!file?.size)throw new Error('اختر صورة أو فيديو');const isImage=file.type.startsWith('image/'),isVideo=file.type.startsWith('video/');if(!isImage&&!isVideo)throw new Error('نوع الملف غير مدعوم');if(isImage&&file.size>5*1024*1024)throw new Error('حجم الصورة أكبر من 5 MB');if(isVideo&&file.size>30*1024*1024)throw new Error('حجم الفيديو أكبر من 30 MB');let uploadFile=file,ext=(file.name.split('.').pop()||'bin').toLowerCase();if(isImage){uploadFile=await compressImage(file);ext='webp'}const path=`ads/${new Date().toISOString().slice(0,10)}/${crypto.randomUUID()}.${ext}`;const {error}=await client.storage.from(cfg.bucket).upload(path,uploadFile,{cacheControl:'31536000',contentType:uploadFile.type,upsert:false});if(error)throw error;const {data}=client.storage.from(cfg.bucket).getPublicUrl(path);return {url:data.publicUrl,type:isVideo?'video':'image'}},
