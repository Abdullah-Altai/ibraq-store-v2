...
function renderAds(){
 ...
 $("#adsSlider").innerHTML=ads.map(a=>{const isVideo=a.mediaType==='video'||/\.(mp4|webm|mov)(\?|$)/i.test(a.image||'');return `<div class="ad-card" data-ad="${a.id}">${isVideo?`<video src="${a.image}" autoplay muted loop playsinline preload="metadata"></video>`:`<img src="${a.image}" loading="eager" decoding="async">`}<div class="ad-overlay"><h3>${esc(loc(a,"title"))}</h3><p>${esc(loc(a,"subtitle"))}</p></div></div>`}).join("");
